...
def train(g, model):
    ...
    print('loss: {:.3f}, val acc: {:.3f}, test acc: {:.3f}'.format(loss, val_acc, test_acc))

def main(argv,g,dataset):
   option = ''
   try:
      opts, args = getopt.getopt(argv,"hr:",["run="])
   except getopt.GetoptError:
      print(len(sys.argv))
      print('introduction.py -run <option> or introduction --run <option>')
      sys.exit(2)
   for opt, arg in opts:
      if opt == '-h':
         print('usage: introduciton.py -r <option> or introduction.py --run <option>')
         sys.exit()
      elif opt in ("-r", "--run"):
         option = arg
   if(option == 'cuda'):
     print('program running on cuda')
     g = g.to('cuda')
     ...
     print('Error: Invalid input argument. expected \'cpu\' or \'cuda\'')

if __name__ == "__main__":
   main(sys.argv[1:],g,dataset)
